...
import { ArrowLeft, Package, Calendar as CalendarIcon, User, Phone, Mail, MapPin, Building, Briefcase, Loader2, Hash, Truck, DollarSign, Tag, Home, Flag } from "lucide-react";
...
export const ColetaForm: React.FC<ColetaFormProps> = ({ initialData, onSave, onCancel, isPending }) => {
  ...
  const fetchAddressByCep = async (cep: string, isOrigin: boolean) => {
    ...
    try {
      ...
      // Update the main 'endereco' field for backward compatibility/display if it's the origin
      if (isOrigin) {
  ...
  const handleClientComboboxSelect = (client: Client | null) => {
    if (client) {
      ...
      if (client.address) {
        setEnderecoOrigemInput(client.address);
