...
import { BarChart3, FileText, TrendingUp, Package, CheckCircle } from "lucide-react";
import {
  ResponsiveContainer,
  AreaChart,
  XAxis,
...
const dummyPerformanceData = [
  { month: format(startOfMonth(new Date(2025, 0)), 'MMM', { locale: ptBR }), totalColetas: 0, totalItems: 0 },
  { month: format(startOfMonth(new Date(2025, 1)), 'MMM', { locale: ptBR }), totalColetas: 0, totalItems: 0 },
  { month: format(startOfMonth(new Date(2025, 2)), 'MMM', { locale: ptBR }), totalColetas: 0, totalItems: 0 },
  { month: format(startOfMonth(new Date(2025, 3)), 'MMM', { locale: ptBR }), totalColetas: 0, totalItems: 0 },
  { month: format(startOfMonth(new Date(2025, 4)), 'MMM', { locale: ptBR }), totalColetas: 0, totalItems: 0 },
  { month: format(startOfMonth(new Date(2025, 5)), 'MMM', { locale: ptBR }), totalColetas: 0, totalItems: 0 },
  { month: format(startOfMonth(new Date(2025, 6)), 'MMM', { locale: ptBR }), totalColetas: 0, totalItems: 0 },
  { month: format(startOfMonth(new Date(2025, 7)), 'MMM', { locale: ptBR }), totalColetas: 2, totalItems: 3 }, // Agosto
  { month: format(startOfMonth(new Date(2025, 8)), 'MMM', { locale: ptBR }), totalColetas: 5, totalItems: 8 }, // Setembro
  { month: format(startOfMonth(new Date(2025, 9)), 'MMM', { locale: ptBR }), totalColetas: 3, totalItems: 4 }, // Outubro
  { month: format(startOfMonth(new Date(2025, 10)), 'MMM', { locale: ptBR }), totalColetas: 0, totalItems: 0 },
  { month: format(startOfMonth(new Date(2025, 11)), 'MMM', { locale: ptBR }), totalColetas: 0, totalItems: 0 },
];

export const CollectionReportDashboardCard: React.FC = () => {
  const selectedYear = '2025'; // Ano fictício para o dashboard

  // Dados fictícios para as métricas
  const dashboardMetrics = [
    { titulo: "Coletas Finalizadas", valor: "1", unidade: "", variacao: "+12%", icon: CheckCircle, color: 'text-success-green', bgColor: 'bg-success-green/10' },
