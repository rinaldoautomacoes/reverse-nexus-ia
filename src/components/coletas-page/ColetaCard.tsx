...
import {
  Edit, Trash2, Package, Clock, Truck, CheckCircle, User, MapPin, Hash, Calendar as CalendarIcon, Building, MessageSquare, Send, DollarSign, Tag, Home, Flag, ClipboardList, FileText, Paperclip
} from "lucide-react";
...
interface ColetaCardProps {
  ...
  onDelete: (coletaId: string) => void;
  onUpdateStatus: (coletaId: string, name: string, status: string) => void;
  onUpdateResponsible: (coletaId: string, name: string, responsibleUserId: string | null) => void;
  onWhatsAppClick: (coleta: Coleta) => void;
  onEmailClick: (coleta: Coleta) => void;
  onViewAttachments: (attachments: FileAttachment[], collectionName: string) => void;
  isDeleting: boolean;
}
...
export const ColetaCard: React.FC<ColetaCardProps> = ({
  ...
  const validAttachments = coleta.attachments?.filter(file => 
          ...
          {validAttachments.length > 0 && (
            <div className="flex items-center gap-1 col-span-full">
              ...
              <Button
                ...
                onClick={() => onViewAttachments(validAttachments, coleta.parceiro || 'Coleta')}
              >
                Ver Anexos
              </Button>
            </div>
