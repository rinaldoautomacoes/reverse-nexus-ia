...
export const AgendarEntrega = () => {
  ...
  const addEntregaMutation = useMutation({
    mutationFn: async (newEntregaData: ColetaInsert) => { // Recebe um objeto ColetaInsert já formatado
      if (!user?.id) {
        throw new Error("Usuário não autenticado. Faça login para agendar entregas.");
      }
      const { data, error } = await supabase
        .from('coletas') // Reusing 'coletas' table for deliveries
        .insert({ ...newEntregaData, user_id: user.id, type: 'entrega' }) // Garantir que o tipo seja 'entrega'
        .select()
        .single();
    ...
    onSuccess: async (newEntrega) => {
      ...
      if (newEntrega.id && session?.access_token) {
        try {
          await supabase.functions.invoke('send-notification', {
            body: JSON.stringify({ collectionId: newEntrega.id, type: 'entrega' }),
            headers: {
          ...
          toast({
            ...
            description: "O responsável foi notificado sobre a nova entrega.",
          });
      ...
      setFormData({
        ...
        modelo_aparelho: "",
        qtd_aparelhos_solicitado: 0,
        status_coleta: "pendente", // Resetar para 'pendente'
        observacao: "",
        user_id: user?.id || '',
        responsible_user_id: null,
  ...
  const addClientMutation = useMutation({
    ...
    onSuccess: (newClient) => {
      ...
      // Preenche o formulário de entrega com os dados do novo cliente
      setFormData(prev => ({ 
        ...prev, 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user?.id) {
      toast({
        ...
        description: "Você precisa estar logado para agendar uma entrega.",
        variant: "destructive"
      });
      return;
    }
    ...
    const { product_description, ...entregaDataToInsert } = formData;

    // Agora, agende a entrega com o client_id e os dados adicionais
    addEntregaMutation.mutate({ 
      ...entregaDataToInsert, // Usa o objeto sem product_description
      user_id: user.id, 
        ...
        <div className="space-y-6">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">
              Configure uma nova entrega com otimização inteligente de rota
            </p>
          </div>
