...
export const Relatorios = () => {
  ...
  const { data: itemsData, isLoading: isLoadingItems, error: itemsError } = useQuery<Item[], Error>({
    ...
    queryFn: async () => {
      ...
      if (coletasData) {
        const relevantCollectionIds = coletasData.filter(c => c.type === 'coleta').map(c => c.id);
  ...
  const { data: reports, isLoading: isLoadingReports, error: reportsError } = useQuery<Report[], Error>({
    ...
    queryFn: async () => {
      ...
      let query = supabase
        ...
        .select('*')
        .eq('user_id', user.id)
        .eq('collection_type_filter', 'coleta'); // Filtrar relatórios que são especificamente para 'coleta'
  ...
  const handleDownload = async (report: Report) => {
    ...
    try {
      ...
      await generateReport({ ...report, collection_type_filter: 'coleta' }, user.id, performanceChartData); // Passar performanceChartData e o filtro de tipo
        ...
        <div className="space-y-6">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">
              Análises inteligentes e insights automatizados para suas coletas
            </p>
          </div>
